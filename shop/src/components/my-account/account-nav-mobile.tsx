    ...
    <Listbox value={selectedItem} onChange={handleItemClick}>
      {({ open }) => (
        <div className="relative w-full font-body">
          <Listbox.Button className="relative w-full p-4 md:p-5 text-start  rounded focus:outline-none cursor-pointer border border-gray-300 bg-gray-200 flex items-center">
            {selectedItem?.icon}
            <span className="flex truncate items-center text-sm md:text-[15px] ps-2.5 relative text-heading font-semibold">
              {t(selectedItem?.name)}
            </span>
